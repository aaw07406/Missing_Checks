...
def process_query():
    ...
    try:
        ...
        merged_df.sort_values(by='CHECK_AMT', ascending=False, inplace=True)

        print("DataFrame after merge:")
        print(merged_df.head())
        print('Adding back in zeros to missing checks...')
        # Add leading zeros back to the CHECK_NUM column
        merged_df['CHECK_NUM'] = merged_df['CHECK_NUM'].astype(str).str.zfill(12)
        ...
        # Select relevant columns for display
            merged_df = merged_df[['RECORD_TYPE_RF', 'CARRIER_CD', 'CARRIER_NM', 'CHECK_NUM', 'CHECK_AMT']]
            print(merged_df.head())
...
def export_file():
    ...
    try:
        if file_ext == "csv":
            ...
            # Use xlsxwriter to preserve formatting
            with pd.ExcelWriter(file_path, engine="xlsxwriter") as writer:
                ...
                # Format CHECK_NUM column as text
                text_format = workbook.add_format({'num_format': '@'})

                # Find the column index of 'CHECK_NUM'
                if "CHECK_NUM" in merged_df.columns:
                    col_idx = merged_df.columns.get_loc("CHECK_NUM")
                    excel_col_letter = chr(65 + col_idx)  # Convert index to Excel column (A-Z only)
                    col_range = f"{excel_col_letter}:{excel_col_letter}"
                    worksheet.set_column(col_range, 20, text_format)

        messagebox.showinfo("Export Successful", f"File exported successfully as {file_ext.upper()}. Data exported to Teradata table.")
...
td_icon = CTkImage(Image.open("images/td_icon.png"), size=(20, 20))
...
td_export_button = ctk.CTkButton(button_frame, text="TD Export", image=td_icon, compound='left', command=export_td_file, font=("Segoe UI", 14, "bold"),
                                 corner_radius=5,
                                 border_width=1.5,
                                 fg_color="#1c8c53",         # Deep green
                                 hover_color="#29a06d",      # Lighter on hover
                                 border_color="#ffffff",     # White border
                                 text_color="white")
td_export_button.grid(row=0, column=2, padx=10)
